...
def get_latest_btc_data(days=30):
    ...
    # 创建数据获取器
    fetcher = BinanceHistoricalFetcher(symbol="BTCUSDT", interval="4h")
...
def generate_future_timestamps(last_timestamp, pred_len, interval='4h'):
    """
    生成未来的时间戳
    """
    future_timestamps = []
...
def predict_btc_prices(tokenizer, model, input_data, pred_len=24):
    """
    预测BTC价格
    """
    print(f"正在预测未来{pred_len * 4}小时的BTC价格（{pred_len}个4小时周期）...")
    ...
    y_timestamp = generate_future_timestamps(last_timestamp, pred_len, '4h')
...
def visualize_results(historical_data, predicted_data, lookback_window=100):
    ...
    plot_path = f"btc_4h_prediction_{timestamp}.png"
...
def save_prediction_results(historical_data, predicted_data):
    ...
    # 保存预测结果
    pred_path = f"btc_4h_prediction_{timestamp}.csv"
    ...
    combined_path = f"btc_4h_combined_{timestamp}.csv"
...
def main():
    ...
    try:
        ...
        # 2. 获取最新数据
        df = get_latest_btc_data(days=120)
