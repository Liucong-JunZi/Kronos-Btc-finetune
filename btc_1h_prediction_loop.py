...
tokenizer = None
model = None
update_interval = 3600  # 默认1小时（秒）
is_predicting = False  # 添加预测状态标志
...
def update_graph(n):
    """更新图表回调函数 - 每次刷新都会触发真实的预测"""
    global global_df, global_predicted, update_interval, tokenizer, model, is_predicting
    ...
    # === 关键修改：每次回调都重新预测 ===
    if n > 0 and not is_predicting:  # n>0 表示不是首次加载
        try:
            is_predicting = True
            ...
            # 4. 更新全局变量
            global_df = df
            global_predicted = predicted_data
    ...
    stats = html.Div([
        ...
        html.Div([
            ...
            html.Span(f' | 🔄 刷新次数: {n}', 
                     style={'color': '#888', 'fontSize': '14px', 'marginLeft': '20px'})
        ])
    ])
...
# 简化 prediction_loop - 只负责首次加载
def prediction_loop():
    """初始化预测（只运行一次）"""
    global tokenizer, model, global_df, global_predicted
...
if __name__ == "__main__":
    ...
    print("⏰ 默认每60分钟自动刷新并重新预测")
    print("🔄 可在网页上调整刷新间隔（1-1440分钟）")
    print("⚡ 每次刷新都会：")
    print("   1️⃣  重新获取最新30天数据")
    print("   2️⃣  通过模型重新预测48小时")
    print("   3️⃣  更新K线图显示")
    print("="*70 + "\n")
